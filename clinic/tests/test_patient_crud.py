...
class PatientCRUDViewTests(TestCase):
    ...
    def test_patient_detail_research_cockpit_and_tools_labels(self) -> None:
        ...
        self.assertContains(response, "Research workflow next step")
    ...
    def test_patient_detail_shows_research_interpretation_status_before_simulation_launcher(self) -> None:
        ...
        self.assertContains(response, "A cross-scenario ordering is undefined until a research utility function is specified.")
    ...
    def test_patient_prognosis_timeline_renders_without_clinical_overclaim(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "PATIENT_SPECIFIC_PREDICTION_NOT_VALIDATED")
        self.assertContains(response, "INSUFFICIENT_VALIDATION")
        self.assertContains(response, "No patient-specific probability is emitted at E1")
        self.assertContains(response, "What can be concluded")
        self.assertContains(response, "What cannot be concluded")
        self.assertContains(response, "Available descriptive lineage")
        self.assertContains(response, "Open Simple Research View")
        self.assertContains(response, "Open Scientific Cockpit")
        self.assertNotContains(response, "83%")
        self.assertNotContains(response, "PFS probability")
    ...
    def test_patient_regimen_suggestions_use_exploratory_language(self) -> None:
        ...
        self.assertContains(response, "NEEDS_EVIDENCE")
        self.assertContains(response, "What can be concluded")
        self.assertContains(response, "What cannot be concluded")
